...
import { formatEventDateLabel, formatZarLabel, groupPurchaseLogRows, moneyZar } from './eventPurchaseLog.js';
...
function addPurchasesTable(ws, tableName, rows, startRow = 4) {
  ...
  ws.addTable({
    ...
    columns: TABLE_COLUMNS.map((col) => ({
      name: col.name,
      filterButton: true,
      ...(col.money ? { totalsRowFunction: 'sum' } : { totalsRowLabel: col.name === 'Guest name' ? 'Total' : '' }),
    })),
  ...
  for (let r = firstData; r <= lastData + 1; r += 1) {
    ws.getCell(r, amountCol).numFmt = MONEY_FORMAT;
  }
}
...
export async function rowsToXlsxBuffer({ eventTitle, eventDate, rows = [], groups: groupsIn = null, totalZar = null } = {}) {
  ...
  const summary = [dateLabel, `${count} ${count === 1 ? 'purchase' : 'purchases'}`, `Total R${formatZarLabel(total)}`]
    .filter(Boolean)
    .join('   ·   ');
  ...
  summaryWs.getCell(`A${r}`).value = 'Total';
